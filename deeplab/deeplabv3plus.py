...
class DeepLabv3Plus(nn.Module):
    ...
    def __init__(self, n_channels=3, n_classes=21, os=16, pretrained=False, _print=True, backbone='resnet50',
                 bilinear=True):
        assert backbone in ['resnet18', 'resnet34', 'resnet50', 'resnet101'], 'backbone not supported'
        if _print:
        ...
        if backbone == 'resnet18':
            self.resnet_features = ResNet18(n_channels, os, pretrained=pretrained)
            self.backbone_out_channels = 512
            self.low_level_out_channels = 64
        elif backbone == 'resnet34':
            self.resnet_features = ResNet34(n_channels, os, pretrained=pretrained)
            self.backbone_out_channels = 512
            self.low_level_out_channels = 64
        elif backbone == 'resnet50':
            self.resnet_features = ResNet50(n_channels, os, pretrained=pretrained)
            self.backbone_out_channels = 2048
            self.low_level_out_channels = 256
        ...
        self.aspp1 = ASPP_module(self.backbone_out_channels, 256, rate=rates[0])
        self.aspp2 = ASPP_module(self.backbone_out_channels, 256, rate=rates[1])
        self.aspp3 = ASPP_module(self.backbone_out_channels, 256, rate=rates[2])
        self.aspp4 = ASPP_module(self.backbone_out_channels, 256, rate=rates[3])

        self.relu = nn.ReLU()

        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                             nn.Conv2d(self.backbone_out_channels, 256, 1, stride=1, bias=False),
                                             nn.BatchNorm2d(256),
                                             nn.ReLU())
        ...
        self.conv2 = nn.Conv2d(self.low_level_out_channels, 48, 1, bias=False)
...
if __name__ == "__main__":
    model = DeepLabv3Plus(n_channels=3, n_classes=21, os=16, pretrained=True, _print=True, backbone='resnet18')
